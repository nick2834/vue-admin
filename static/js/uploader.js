(function() {
  ...
  var DEFAULT_IMG = ['jpg', 'jpeg', 'gif', 'png', 'bmp','flv','mpg','mpeg','avi','wmv','mov','asf','rm','rmvb','mkv','m4v','mp4'];
  ...
  var qnUpload = function(options) {
    ...
    this.defaultSize = parseInt(options.maxImgSize, 10) || 10 * 1024;
    this.uploadUrl = 'https://up.qbox.me/?token=';
  ...
  qnUpload.prototype.post = function(file, cb) {
    ...
    this._postPrepare(function() {
      ...
      data.append('key', self.prefix + randName + file.name);
      var postUrl = self.uploadUrl + self.token.data.token + '&rand=' + Math.random(); // avoid cache
      var options = {
        method: 'POST',
        url: postUrl,
        onProgress: true,
      ...
      request.call(self, options, function(err, returnObj) {
        ...
        var fileInfo = {};
          fileInfo.fileUrl = '/' + returnObj.key;
        // imageUrlInfo.fullImageUrl = self.domain + '/' + returnObj.key + file.name;
          fileInfo.mobileFileUrl = '/' + returnObj.key + '?imageView2/0/w/' + self.maxMobileWidth
          fileInfo.pcFileUrl = '/' + returnObj.key + '?imageView2/2/w/' + self.maxPcWidth;
        cb(fileInfo);
      });
    });
  }
